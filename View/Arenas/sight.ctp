...
<script>
    ...
    // Afficher les ennemis ds leurs positions respectives
    function showImageAtPosition(x, y, name)
    {
        ...
        //savePosition();
        putCacheToEnemiesOutOfMisight(true);
    ...
    // Attribuer une pos aléatoire à tous les fighters
    ...
    // On affiche en gris l'arrière plan
    function putCacheToEnemiesOutOfMisight($save_only)
    {
        if ($save_only === true) {
            $.get(
                    '<?php echo Router::url(array('controller' => 'arenas', 'action' => 'InitialisePositionFighter')); ?>',
            ...
            $.get(
                    '<?php
    echo Router::url(array('controller' => 'arenas', 'action' => 'save_coordinate_and_show_ennemies'));
    ?>',
                    {coord_x: $('#image').parent().attr('id'),
                        coord_y: $('#image').parent().parent().attr('id')},
            function (data) {
                ...
                if (data !== '' || data !== undefined) {
                    $.each(data, function (i, item) {
                        ...
                        if (item['ennemy_name'] !== myFighterName)
                            alert('enemie à portée :=>  ' + item['ennemy_name'] + ' skill_used ' + item['sight_used']);
                        $('.imagesEnemies').each(function () {
                            //      alert(item +' : '+ $(this).attr('name') );

                            if (item['ennemy_name'] !== $(this).attr('name'))
                            {
                                //  alert('j"add le cahs');
                                enemieToAttackName = item['ennemy_name'];
                                $(this).parent().addClass('putCash');

                            }
                            else
                            {
                                if ($(this).attr('name') === item['ennemy_name'])
                                {
                                    //  alert('je suppr');
                                    enemieToAttackName = $(this).attr('name');
    ...
    function showEnemies()
    {
        $.get(
                '<?php
    echo Router::url(array('controller' => 'arenas', 'action' => 'showEnemies'));
    ?>',
                {fighter_name: $('#image').attr('name')},
        function (data) {
            //     alert('yes');
            //   alert(data[1]['Fighter']['name']);               // $('#').text(data);
            $.each(data, function (i, item) {
                $.each(item, function (k, dat) {
                    k = i;
                    //   alert(k +' => ' + dat['name']);
                })
        ...
        moveCombattant: function (e) {
            ...
            switch (e.keyCode)
            {
                case 37 :  // gauche
                    ...
                    //savePosition();
                    putCacheToEnemiesOutOfMisight(false);
                    break;
    ...
    $(document).ready(putCacheToEnemiesOutOfMisight());
    ...
    $('#gameInterface').children('tbody').find('tr').find('th').click(function () {
        // alert('click done : ' + '('+ $(this).attr('id') +','+ $(this).parent().attr('id')+')');
        // alert(attackPossible);

        if (attackPossible == true && enemieToAttackName == $(this).children('img').attr('name'))
        {
            alert('enemi to attack :' + enemieToAttackName);
            alert('attack reussi');
            //$('#'+ enemieToAttackName).remove();
            updateTableUser(enemieToAttackName);
        } else if (enemieToAttackName != $(this).children('img').attr('name')) {
